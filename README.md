...
./logsan sanitize --in ./logs --out ./clean-logs --mapping-in mapping.json --mapping-out mapping_new.json
...
Ограничения производительности происходят из-за долгой обработки регулярных значений (использование групп необходимо для некоторых детекторов, например токена, путей в системе, но значительно уменьшает скорость обработки; FindAllStringSubmatch и дальнейшие проверки работает дольше, чем FindAllString) и обеспечения сихронизации доступа к словарю замен (использование мьютекса для обработок строк и корректной записи в словарь замен, иначе race condition).
